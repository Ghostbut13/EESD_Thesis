...
void MOESI_SMPCache::readLine(uint32_t rdPC, uint32_t addr){
  ...
  if(!st || (st && !(st->isValid())) ){//Read Miss -- i need to look in other peoples' caches for this data
	...
	// counter
      ...
      filter_ctr[this->getCPUId()] = filter_ctr[this->getCPUId()] +1;
      // base updating
      filter_base[this->getCPUId()] = addr;
    }
	...
	printf("(de)base + mask + ctr + cpuID:  %d  %d   %d,  %d\n",filter_base[this->getCPUId()],filter_mask[this->getCPUId()],filter_ctr[this->getCPUId()],this->getCPUId());
...
MOESI_SMPCache::InvalidateReply MOESI_SMPCache::writeRemoteAction(uint32_t addr){
    ...
    //******************************
	  ...
	  filter_ctr[otherCache->getCPUId()] = filter_ctr[otherCache->getCPUId()] -1;    /// SR is counting down
	  if(filter_ctr[otherCache->getCPUId()] ==  0){ /// if SR_ctr ==0 --> reset it
	    filter_initial_flag[otherCache->getCPUId()] = 1; // reset flag (or initial flag)
	  }
          empty = false;
      }
