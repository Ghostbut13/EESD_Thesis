...
uint16_t filter_base[4]         = {0x7f,0x7f,0x7f,0x7f}; //15bit
uint16_t filter_mask[4]         = {0x00,0x00,0x00,0x00}; //15bit
...
MSI_SMPCache::RemoteReadService MSI_SMPCache::readRemoteAction(uint32_t addr){ //remoteRemoteAction() returns MSI_SMPCache object
  ...
  for(cacheIter = this->getCacheVector()->begin(), lastCacheIter = this->getCacheVector()->end(); 
      ...
      cacheIter++)
    {
      ...
      if(otherCache->getCPUId() == this->getCPUId()){// getCPUId returns int %d

	/*We don't want to snoop our own access*/
	continue; //come back to forloop
	...
	if(otherState->getState() == MSI_MODIFIED){// other is M
	...
	}else if(otherState->getState() == MSI_SHARED){  //other is S
	...
	}else if(otherState->getState() == MSI_INVALID){ //other is I
  ...
  // if all returns fails, it means: all cache are MSI_I
...
void MSI_SMPCache::readLine(uint32_t rdPC, uint32_t addr){
  ...
  if(!st || (st && !(st->isValid())) ){
     ...
     */
    MSI_SMPCache::RemoteReadService rrs = readRemoteAction(addr); //readRemote do be for SMPCache object: rrs
    numReadRequestsSent++;
...
MSI_SMPCache::InvalidateReply MSI_SMPCache::writeRemoteAction(uint32_t addr){
   ...
   */
  return MSI_SMPCache::InvalidateReply(empty); //i think it is the initial state we want
}
