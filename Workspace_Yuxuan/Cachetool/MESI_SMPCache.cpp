...
MESI_SMPCache::RemoteReadService MESI_SMPCache::readRemoteAction(uint32_t addr){//change cache state in this function
  ...
  for(cacheIter = this->getCacheVector()->begin(), lastCacheIter = this->getCacheVector()->end(); cacheIter != lastCacheIter; cacheIter++){
    ...
    if(otherState){
      if(otherState->getState() == MESI_MODIFIED){//change state to shared and provide data.

        otherState->changeStateTo(MESI_SHARED);
        return MESI_SMPCache::RemoteReadService(false,true);

      }else if(otherState->getState() == MESI_EXCLUSIVE){//change state to shared and provide data.


        otherState->changeStateTo(MESI_SHARED); 
        return MESI_SMPCache::RemoteReadService(false,true);//

      }else if(otherState->getState() == MESI_SHARED){  //doesn't matter except that someone's got it//change state to shared and provide data.

        return MESI_SMPCache::RemoteReadService(true,true);

      }else if(otherState->getState() == MESI_INVALID){ //doesn't matter at all. doesn't change anything
...
void MESI_SMPCache::readLine(uint32_t rdPC, uint32_t addr){
  ...
  if(!st || (st && !(st->isValid())) ){//Read Miss -- i need to look in other peoples' caches for this data
    ...
    MESI_SMPCache::RemoteReadService rrs = readRemoteAction(addr);// find cache line in other cache, and change state.
    numReadRequestsSent++;
